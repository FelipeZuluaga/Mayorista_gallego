                                ...
                                return (
                                    <tr key={item.product_id} className={tieneVenta ? "row-active" : ""}>
                                        ...
                                        <td>
                                            <input
                                                type="text"
                                                className={`input-modern price ${precioVenta !== "" && Number(precioVenta) < (Number(item.unit_price) + 2000)
                                                    ? "input-error-margen"
                                                    : ""
                                                    }`}
                                                placeholder={`Min: $${Number(item.unit_price) + 2000}`}
                                                value={precioVenta}
                                                onChange={(e) => {
                                                    const val = e.target.value;
                                                    // Permitimos escribir cualquier número para no bloquear el teclado
                                                    if (val === "" || /^[0-9]+$/.test(val)) {
                                                        updatePrecioVenta(item.product_id, val);
                                                    }
                                                }}
                                                onBlur={(e) => {
                                                    const min = Number(item.unit_price) + 2000;
                                                    if (e.target.value !== "" && Number(e.target.value) < min) {
                                                        alertError("Precio no permitido", `El precio mínimo de venta debe ser $${min.toLocaleString()}`);
                                                        updatePrecioVenta(item.product_id, ""); // Limpia el valor si es inválido
                                                    }
                                                }}
                                            />
                                        </td>
...
export default function VentasPage() {
    ...
    const updatePrecioVenta = (prodId, nuevoPrecio) => {
        // Permitimos que escriba cualquier número o que borre el campo
        if (nuevoPrecio === "" || /^[0-9]+$/.test(nuevoPrecio)) {
            const nuevaPlanilla = [...planilla];
    ...
    // --- FUNCIÓN PARA GENERAR EL PDF (OPTIMIZADA PARA 58MM) ---
    const generarPDFVenta = (cliente) => {
        // 1. CONFIGURACIÓN REAL PARA POS-58 (Ancho real de impresión suele ser 48-52mm)
        const doc = new jsPDF({
            unit: "mm",
            format: [58, 150] // Cambiado de 80 a 58
        });
        ...
        const pageWidth = doc.internal.pageSize.getWidth(); // Esto ahora será 58

        // 2. ENCABEZADO (Ajustado el tamaño de fuente para 58mm)
        doc.setFontSize(10); // Bajamos de 12 a 10 para que no se corte
        ...
        doc.text(`FECHA: ${new Date().toLocaleString()}`, 4, 16);
        doc.text(`VEND: ${selectedOrder.seller_name.toUpperCase()}`, 4, 20);
        doc.text(`CTE: ${cliente.name.toUpperCase()}`, 4, 24);
        doc.text(`DIR: ${cliente.address.substring(0, 28)}`, 4, 28); // Menos caracteres por el ancho
        doc.text("---------------------------------------", pageWidth / 2, 32, { align: "center" });

        // 3. TABLA DE PRODUCTOS (Ajustamos anchos de columna para 58mm)
        const tableRows = [];
        Object.entries(cliente.productos).forEach(([id, cant]) => {
            if (cant > 0) {
                ...
                tableRows.push([
                    p.product_name.substring(0, 18), // Acortamos el nombre para que no se amontone
                    cant,
        ...
        autoTable(doc, {
            ...
            styles: {
                ...
                textColor: [0, 0, 0] // Forzamos negro puro
            },
            headStyles: { fontStyle: 'bold' },
            columnStyles: {
                0: { cellWidth: 24 }, // Nombre producto
                1: { cellWidth: 8, halign: 'center' }, // Cantidad
                2: { cellWidth: 18, halign: 'right' }  // Total
            },
            head: [['ARTICULO', 'CANT', 'TOTAL']],
            body: tableRows,
            margin: { left: 2, right: 2 } // Márgenes mínimos para aprovechar el papel
        });

        // 4. RESUMEN FINAL
        let finalY = doc.lastAutoTable.finalY + 4;
        const rightAlign = pageWidth - 4;

        doc.setFontSize(7);
        doc.text("---------------------------------------", pageWidth / 2, finalY, { align: "center" });
        finalY += 4;

        doc.text("DEUDA ANTERIOR:", 4, finalY);
        doc.text(`$${deudaAnterior.toLocaleString()}`, rightAlign, finalY, { align: "right" });

        finalY += 4;
        doc.text("VENTA HOY (+):", 4, finalY);
        doc.text(`$${totalVentaHoy.toLocaleString()}`, rightAlign, finalY, { align: "right" });

        finalY += 4;
        doc.text("PAGO RECIBIDO (-):", 4, finalY);
        doc.text(`$${abonoEfectivo.toLocaleString()}`, rightAlign, finalY, { align: "right" });

        finalY += 6;
        doc.setFontSize(9); // Un poco más grande para el total
        doc.setFont("helvetica", "bold");
        doc.text("SALDO TOTAL:", 4, finalY);
        ...
        <div className="ventas-container">
            ...
            {!selectedOrder ? (
                <div className="ruta-selection">
                    ...
                    {/* PLANILLA DE RUTA PARA COMENZAR A VENDER */}
                </div>
            ) : (
                                        ...
                                        return (
                                            <tr key={idx} className={estadoClase}>
                                                ...
                                                <td className="name-col">
                                                    {cliente.phone ? (
                                                        <button
                                                            onClick={() => abrirWhatsApp(cliente.phone)}
                                                        ...
                                                        <span style={{ color: '#ccc' }}>Sin tel.</span>
                                                    )}
                                                </td>
                                            </tr>
                                        );
